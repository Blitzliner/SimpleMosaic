...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--tile_multiplier', default=20, help='How many tiles should be in one row/column')
    ...
    db = Mosaic.TileDatabase(image_dir=args.image_dir,
                             database_file=args.database_file,
                             tile_size_ratio=args.tile_ratio,
                             tile_max_width=args.tile_width)
    db.create()

    tf = Mosaic.TileFitter(overlay_image_path=args.overlay_image_path,
                           database_file=args.database_file,
